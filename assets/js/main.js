...
const formElement = document.getElementById("userID");
formElement.addEventListener("submit", (event) => {
  event.preventDefault();
  let username = document.getElementaryId("transactionDescription").value;
  let password = document.getElementById("transactionContra").value;
  let user = { username: username, password: password}
  let userIDJson = JSON.stringify(user);
...
// actualizar carrito lazy loading
function updateCart() {
...
function addToCart(productId) {
  ...
  if (product) {
    ...
    .then(response => {
      if (response.ok) {
        console.log("Producto enviado al backend con éxito:", productData);
      } else {
